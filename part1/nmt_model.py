...
class NMT(nn.Module):
    ...
    def step(
        ...
        # 2b. Apply mask (skip when enc_masks is None, e.g. in greedy decoding)
        if enc_masks is not None:
            # enc_masks: 1 at PAD positions → set their scores to -inf
        ...
        context_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)

        # 5. Concatenate dec_hidden with attention_t to compute tensor u_t
        u_t = torch.cat([dec_hidden, context_t], dim=1)
